#!/usr/bin/env python
#
# wxCSV.py
#
# A first pass at downloading the current and forecast weather 
# and sending it in csv format as an output.
#
# Seth McNeill
# 2013 October 21

import sys
import datetime
from urllib2 import urlopen
# need to have import try/excepts here
from lxml import etree 
import ForeWx_api as ForeSupermod # generated by generateDS
import ForeWx_proc # generated by generateDS
import curWx_api as CurSupermod # generated by generateDS
import curWx_proc # generated by generateDS
from wxSQL import Currentobs
from wxSQL import Forecast

curURL = 'http://w1.weather.gov/xml/current_obs/KLNK.xml'
forecastURL = 'http://forecast.weather.gov/MapClick.php?lat=40.80686&lon=-96.68168&FcstType=digitalDWML'

def processForecast(inXML, N):
  rootObj = ForeSupermod.parseString(inXML, True)
  print "----------------------------------------------"
  print "Forecast Length (hrs): " + str(len(rootObj.data[0].parameters[0].temperature[0].value))
  print "Date: " + rootObj.head.product.creation_date.get_valueOf_()
  pyObsTime = datetime.datetime.strptime(rootObj.head.product.creation_date.get_valueOf_()[:-6], 
    "%Y-%m-%dT%H:%M:%S")
  print "Location: " + rootObj.data[0].location[0].city.get_valueOf_()
  print "Multiple Prediction Test"
  for ii in range(0,len(rootObj.data[0].parameters[0].temperature[0].value)):
    forecast = Forecast()
    forecast.forecasttime = pyObsTime
    forecast.location = rootObj.data[0].location[0].city.get_valueOf_()
    print "Time: " + rootObj.data[0].time_layout[0].start_valid_time[ii].get_valueOf_(),
    starttime = datetime.datetime.strptime(rootObj.data[0].time_layout[0].start_valid_time[ii].get_valueOf_()[:-6], 
      "%Y-%m-%dT%H:%M:%S")
    forecast.starttime = starttime
    print "to " + str(rootObj.data[0].time_layout[0].end_valid_time[ii])
    endtime = datetime.datetime.strptime(str(rootObj.data[0].time_layout[0].end_valid_time[ii])[:-6], 
      "%Y-%m-%d %H:%M:%S")
    forecast.stoptime = endtime
    for t in rootObj.data[0].parameters[0].temperature:
      print "Temp type " + t.get_type(),
      if(t.value[ii].get_valueOf_() == ''):
        v = None
      else:
        v = t.value[ii].get_valueOf_()
      if(t.get_type() == 'dew point'):
        forecast.dewpoint = v
      if(t.get_type() == 'wind chill'):
        forecast.windchill = v
      if(t.get_type() == 'hourly'):
        forecast.temperature = v
      print v
    print "precipitation probability: " + rootObj.data[0].parameters[0].probability_of_precipitation[0].value[ii].get_valueOf_()
    if(rootObj.data[0].parameters[0].probability_of_precipitation[0].value[ii].get_valueOf_() == ''):
      forecast.precipprob = None
    else:
      forecast.precipprob = rootObj.data[0].parameters[0].probability_of_precipitation[0].value[ii].get_valueOf_()
    for w in rootObj.data[0].parameters[0].wind_speed:
      print "wind " + w.get_type() + ': ' + w.value[ii].get_valueOf_()
      if(w.get_type() == 'sustained'):
        forecast.windspeed = w.value[ii].get_valueOf_()
        if(w.value[ii].get_valueOf_() == ''):
          forecast.windspeed = None
          print "no wind speed value"
        else:
          forecast.windspeed = w.value[ii].get_valueOf_()
      if(w.get_type() == 'gust'):
        if(w.value[ii].get_valueOf_() == ''):
          forecast.windgust = None
          print "no wind gust value"
        else:
          forecast.windgust = w.value[ii].get_valueOf_()
    for d in rootObj.data[0].parameters[0].direction:
      print "direction of " + d.get_type() + ": " + d.value[ii].get_valueOf_()
      if(d.get_type() == 'wind'):
        if(d.value[ii].get_valueOf_() == ''):
          forecast.winddir = None
          print "no wind dir value"
        else:
          forecast.winddir = d.value[ii].get_valueOf_()
    forecast.save()

def processCurrent(inXML):
  rootObj = CurSupermod.parseString(inXML, True)
  curObs = Currentobs() # current observation sql object
  print "Current Observation"
  print "Station ID: " + rootObj.station_id
  curObs.station = rootObj.station_id
  print "Observation Time: " + rootObj.observation_time_rfc822
  #pyObsTime = datetime.datetime.strptime(rootObj.observation_time_rfc822, "Last Updated on %b %d %Y, %I:%M %p %Z")
  pyObsTime = datetime.datetime.strptime(rootObj.observation_time_rfc822[:-6], "%a, %d %b %Y %H:%M:%S")
  curObs.obstime = pyObsTime
  print "Current Temp: " + str(rootObj.temp_f) + " F"
  curObs.temperature = rootObj.temp_f
  print "Rel Humidity: " + str(rootObj.relative_humidity) + "%"
  curObs.humidity = rootObj.relative_humidity
  print "Wind: " + str(rootObj.wind_mph) + " mph at " + str(rootObj.wind_degrees) + " degrees gusting " + str(rootObj.wind_gust_mph) + " mph"
  curObs.windspeed = rootObj.wind_mph
  curObs.winddir = rootObj.wind_degrees
  curObs.windgust = rootObj.wind_gust_mph
  #if(curObs.windgust is None):
  #  curObs.windgust = 0
  print "Pressure: " + str(rootObj.pressure_in) + " in or " + str(rootObj.pressure_mb) + " mb"
  curObs.pressurein = rootObj.pressure_in
  curObs.pressuremb = rootObj.pressure_mb
  print "Current DewPoint: " + str(rootObj.dewpoint_f) + " F"
  curObs.dewpoint = rootObj.dewpoint_f
  print "Windchill: " + str(rootObj.windchill_f) + " F"
  curObs.windchill = rootObj.windchill_f
  print "Visibility: " + str(rootObj.visibility_mi) + " mi"
  curObs.visibility = rootObj.visibility_mi
  curObs.save()
  


USAGE_MSG = """\
   python wxCSV.py
"""

def usage():
  print USAGE_MSG
  sys.exit(1)

def main():
  args = sys.argv[1:]
  if(len(args) != 0):
    usage()
  curXML = urlopen(curURL).read()
  processCurrent(curXML)
  forecastXML = urlopen(forecastURL).read()
  processForecast(forecastXML, 5) 

if __name__ == '__main__':
  main()


